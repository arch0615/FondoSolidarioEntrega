<div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8">
        ...
        <div class="flex items-center space-x-3">
            ...
            <!-- Botón Nueva Escuela -->
            <a href="{{ route('escuelas.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-lg font-medium text-sm text-white hover:bg-primary-700">
        ...
        <div class="bg-white rounded-xl border border-secondary-200 shadow-sm hover:shadow-lg transition-shadow duration-300 flex flex-col">
            <div class="p-6 flex-grow">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <div x-data="{ open: false }" @click.away="open = false" class="relative">
                            ...
                            <div x-show="open" x-transition class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-20 border" style="display: none;">
                                <a href="{{ route('escuelas.show', $escuela->id_escuela) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-secondary-700 hover:bg-secondary-100">
                                    <i class="fas fa-eye fa-fw text-secondary-400"></i> Consultar
                                </a>
                                <a href="{{ route('escuelas.edit', $escuela->id_escuela) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-secondary-700 hover:bg-secondary-100">
                                    <i class="fas fa-pencil-alt fa-fw text-secondary-400"></i> Editar
                                </a>
                                <button wire:click="cambiarEstado({{ $escuela->id_escuela }})" class="flex items-center gap-2 px-4 py-2 text-sm text-secondary-700 hover:bg-secondary-100 w-full text-left">
                                    <i class="fas fa-{{ $escuela->activo ? 'toggle-off' : 'toggle-on' }} fa-fw text-secondary-400"></i> {{ $escuela->activo ? 'Desactivar' : 'Activar' }}
                                </button>
                                <button wire:click="eliminar({{ $escuela->id_escuela }})" wire:confirm="¿Estás seguro?" class="flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 w-full text-left">
                ...
                <div class="mb-4 text-sm text-secondary-700 space-y-1">
                    <p><i class="fas fa-phone-alt text-secondary-400 mr-2 fa-fw"></i>{{ $escuela->telefono ?? 'No disponible' }}</p>
                    <p><i class="fas fa-envelope text-secondary-400 mr-2 fa-fw"></i>{{ $escuela->email ?? 'No disponible' }}</p>
                    <p><i class="fas fa-map-marker-alt text-secondary-400 mr-2 fa-fw"></i>{{ $escuela->direccion ?? 'No disponible' }}</p>
                </div>
                <div class="border-t border-secondary-200 pt-4 mb-4">
                    ...
                    <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                        ...
                        <p>Accidentes: <span class="font-semibold">{{ $escuela->accidentes_count }}</span></p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="mt-8 px-6 py-4 bg-secondary-50 border-t border-secondary-200">
        <div class="flex flex-col sm:flex-row items-center justify-between">
            <div class="text-sm text-secondary-700 mb-4 sm:mb-0">
                @if($escuelas->total() > 0)
                    Mostrando <span class="font-medium text-secondary-900">{{ $escuelas->firstItem() }}</span> a <span class="font-medium text-secondary-900">{{ $escuelas->lastItem() }}</span> de <span class="font-medium text-secondary-900">{{ $escuelas->total() }}</span> resultados
                @else
                    No hay resultados para mostrar
                @endif
            </div>
            @if($escuelas->hasPages())
                {{ $escuelas->links('pagination.custom-tailwind') }}
            @endif
        </div>
    </div>
</div>
...
<script>
    function exportarEscuelas(formato) {
        ...
        switch(formato) {
            case 'csv':
                url = '{{ route("escuelas.export.csv") }}';
                break;
            case 'excel':
                url = '{{ route("escuelas.export.excel") }}';
                break;
            case 'pdf':
                url = '{{ route("escuelas.export.pdf") }}';
                break;
        }
