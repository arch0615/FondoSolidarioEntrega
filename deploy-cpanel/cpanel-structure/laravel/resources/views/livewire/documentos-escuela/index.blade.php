<div>
    <div class="mx-auto px-4">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8">
            ...
            <div class="flex items-center space-x-3">
                ...
                @if(!$isAdmin)
                    <a href="{{ route('documentos-escuela.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-lg font-medium text-sm text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
        ...
        <div class="bg-white rounded-xl border border-secondary-200 mb-6">
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-{{ $isAdmin ? '5' : '4' }} gap-4">
                    <div class="space-y-1">
                    ...
                    @endif
                    <div class="{{ $isAdmin ? 'lg:col-span-5' : 'lg:col-span-4' }} flex justify-end">
                            ...
                            <tr class="hover:bg-secondary-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button wire:click="mostrarEscuelasModal({{ $documento->id_documento }})" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-100 text-primary-800 hover:bg-primary-200 transition-colors duration-200 cursor-pointer">
                                        ...
                                        {{ $documento->escuela ? $documento->escuela->nombre : 'Sin escuela' }}
                                    </button>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    @if($documento->archivo_path)
                                        <a href="{{ Storage::url($documento->archivo_path) }}" target="_blank" class="inline-flex items-center px-3 py-1 text-sm font-medium text-primary-600 bg-primary-50 border border-primary-200 rounded-lg hover:bg-primary-100 hover:text-primary-800 transition-colors duration-200" title="Ver documento">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-secondary-800">{{ $documento->fecha_carga ? $documento->fecha_carga->format('d/m/Y') : 'No especificada' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        @if(!$isAdmin)
                                            <a href="{{ route('documentos-escuela.show', $documento->id_documento) }}" class="p-2 text-secondary-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors duration-200" title="Ver detalles">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                            </a>
                                            <a href="{{ route('documentos-escuela.edit', $documento->id_documento) }}" class="p-2 text-secondary-400 hover:text-warning-600 hover:bg-warning-50 rounded-lg transition-colors duration-200" title="Editar">
                                        ...
                                        @endif
                                        @if($isAdmin && $documento->archivo_path)
    ...
    <script>
        function exportarDocumentos(formato) {
            ...
            switch(formato) {
                case 'csv':
                    url = '{{ route("documentos-escuela.export.csv") }}';
                    break;
                case 'excel':
                    url = '{{ route("documentos-escuela.export.excel") }}';
                    break;
                case 'pdf':
                    url = '{{ route("documentos-escuela.export.pdf") }}';
                    break;
            }
