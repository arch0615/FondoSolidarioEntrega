<div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8">
        <div>
            <h1 class="text-2xl font-semibold text-secondary-900">Gestión de Empleados</h1>
            <p class="mt-1 text-sm text-secondary-600">Administra los empleados de las escuelas</p>
        </div>
        <div class="flex items-center space-x-3">
            ...
            <a href="<?php echo e(route('empleados.create')); ?>" class="inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-lg font-medium text-sm text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
                    ...
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-secondary-500 uppercase tracking-wider">
                             <button wire:click="sortBy('dni')" class="group inline-flex items-center hover:text-secondary-700">
                                DNI/CUIL
                                <!--[if BLOCK]><![endif]--><?php if($sortField === 'dni'): ?>
                                    <!--[if BLOCK]><![endif]--><?php if($sortDirection === 'asc'): ?>
                                        <svg class="ml-2 w-4 h-4 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path></svg>
                                    <?php else: ?>
                                        <svg class="ml-2 w-4 h-4 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                         ...
                         <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-secondary-500 uppercase tracking-wider">
                              <button wire:click="sortBy('cargo')" class="group inline-flex items-center hover:text-secondary-700">
                                 Cargo(s)
                                <!--[if BLOCK]><![endif]--><?php if($sortField === 'cargo'): ?>
                                    <!--[if BLOCK]><![endif]--><?php if($sortDirection === 'asc'): ?>
                                        <svg class="ml-2 w-4 h-4 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path></svg>
                                    <?php else: ?>
                                        <svg class="ml-2 w-4 h-4 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                ...
                <tbody class="bg-white divide-y divide-secondary-200">
                    ...
                    <tr class="hover:bg-secondary-50 transition-colors duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-secondary-900"><?php echo e($empleado->nombre_completo); ?></div>
                        </td>
                         <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-secondary-900">DNI: <?php echo e($empleado->dni ?? 'N/A'); ?></div>
                            <div class="text-sm text-secondary-500">CUIL: <?php echo e($empleado->cuil ?? 'N/A'); ?></div>
                        </td>
                         <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-secondary-900"><?php echo e($empleado->cargo ?? 'N/A'); ?></div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-secondary-900"><?php echo e($empleado->escuela->nombre ?? 'Sin escuela'); ?></div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <!--[if BLOCK]><![endif]--><?php if($empleado->activo): ?>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <div class="flex items-center justify-center space-x-2">
                                <a href="<?php echo e(route('empleados.show', $empleado->id_empleado)); ?>" class="p-2 text-secondary-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors duration-200" title="Ver detalles">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                <a href="<?php echo e(route('empleados.edit', $empleado->id_empleado)); ?>" class="p-2 text-secondary-400 hover:text-warning-600 hover:bg-warning-50 rounded-lg transition-colors duration-200" title="Editar">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                <button wire:click="imprimirBeneficiarios(<?php echo e($empleado->id_empleado); ?>)" class="p-2 text-secondary-400 hover:text-success-600 hover:bg-success-50 rounded-lg transition-colors duration-200" title="Imprimir Beneficiarios SVO">
                                    <i class="fas fa-print"></i>
                                </button>
                                <button wire:click="cambiarEstado(<?php echo e($empleado->id_empleado); ?>)" class="p-2 text-secondary-400 hover:text-info-600 hover:bg-info-50 rounded-lg transition-colors duration-200" title="<?php echo e($empleado->activo ? 'Desactivar' : 'Activar'); ?>">
                                    <!--[if BLOCK]><![endif]--><?php if($empleado->activo): ?>
                                ...
                                <button wire:click="eliminar(<?php echo e($empleado->id_empleado); ?>)" wire:confirm="¿Estás seguro de que deseas eliminar este empleado? Esta acción no se puede deshacer." class="p-2 text-secondary-400 hover:text-danger-600 hover:bg-danger-50 rounded-lg transition-colors duration-200" title="Eliminar">
        ...
        <div class="px-6 py-4 bg-secondary-50 border-t border-secondary-200">
            <div class="flex flex-col sm:flex-row items-center justify-between">
                <div class="text-sm text-secondary-700 mb-4 sm:mb-0">
                    <!--[if BLOCK]><![endif]--><?php if($empleados->total() > 0): ?>
                        Mostrando <span class="font-medium text-secondary-900"><?php echo e($empleados->firstItem()); ?></span> a <span class="font-medium text-secondary-900"><?php echo e($empleados->lastItem()); ?></span> de <span class="font-medium text-secondary-900"><?php echo e($empleados->total()); ?></span> resultados
                    <?php else: ?>
                        No hay resultados para mostrar
                    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                </div>
                <!--[if BLOCK]><![endif]--><?php if($empleados->hasPages()): ?>
                    <?php echo e($empleados->links('pagination.custom-tailwind')); ?>
...
<script>
    function exportarEmpleados(formato) {
        ...
        switch(formato) {
            case 'csv':
                url = '<?php echo e(route("empleados.export.csv")); ?>';
                break;
            case 'excel':
                url = '<?php echo e(route("empleados.export.excel")); ?>';
                break;
            case 'pdf':
                url = '<?php echo e(route("empleados.export.pdf")); ?>';
                break;
        }
    ...
    document.addEventListener('livewire:init', () => {
        Livewire.on('imprimir-beneficiarios', ({ id }) => {
            const url = `<?php echo e(url('empleados')); ?>/${id}/print-beneficiarios`;
            window.open(url, '_blank');
        });
    });
</script>
<?php $__env->stopPush(); ?><?php /**PATH /home/passion/Documents/FondoSolidarioEntrega11/Fondo Solidario Entrega/resources/views/livewire/empleados/index.blade.php ENDPATH**/ ?>