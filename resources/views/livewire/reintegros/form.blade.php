...
<div class="mx-auto px-4">
    ...
    <div class="mb-8">
        <nav class="flex items-center text-sm text-secondary-500 mb-4">
            <a href="{{ route('reintegros.index') }}" class="hover:text-secondary-700">Reintegros</a>
            <svg class="mx-2 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
            <span class="text-secondary-900">{{ $modo == 'create' ? 'Nueva Solicitud de Reintegro' : ($modo == 'edit' ? 'Editar Solicitud' : 'Detalles de la Solicitud') }}</span>
        </nav>
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-secondary-900">
                    {{ $modo == 'create' ? 'Nueva Solicitud de Reintegro' : ($modo == 'edit' ? 'Editar Solicitud' : 'Detalles de la Solicitud') }}
                </h1>
                <p class="mt-1 text-sm text-secondary-600">
                    {{ $modo == 'create' ? 'Complete los datos para solicitar un nuevo reintegro.' : ($modo == 'edit' ? 'Modifique los datos de la solicitud.' : 'Información detallada de la solicitud de reintegro.') }}
                </p>
            </div>
            @if($modo == 'show')
            <div class="flex space-x-3">
                <a href="{{ route('reintegros.edit', $reintegro_id ?? 1) }}" class="inline-flex items-center px-4 py-2 border border-secondary-300 rounded-lg text-sm font-medium text-secondary-700 bg-white hover:bg-secondary-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
    ...
    <div class="bg-white rounded-xl border border-secondary-200">
        <form wire:submit.prevent="save" class="space-y-6 p-6" enctype="multipart/form-data">
            ...
            <div class="border-b border-secondary-200 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1">
                        ...
                        <select wire:model="id_accidente" id="id_accidente" class="block w-full px-3 py-2 border border-secondary-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 {{ $modo == 'show' ? 'bg-secondary-50' : 'bg-white' }}" {{ $modo == 'show' ? 'disabled' : '' }} required>
                    ...
                    <div class="space-y-1">
                        ...
                        <select wire:model="tipo_gasto" id="tipo_gasto" class="block w-full px-3 py-2 border border-secondary-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 {{ $modo == 'show' ? 'bg-secondary-50' : 'bg-white' }}" {{ $modo == 'show' ? 'disabled' : '' }} required>
            ...
            <div class="border-b border-secondary-200 pb-6">
                ...
                <div id="lista_archivos_container">
                    <label class="block text-sm font-medium text-secondary-700 mb-2">
                        {{ $modo == 'show' ? 'Documentos del Reintegro:' : 'Archivos Seleccionados:' }}
                    </label>
                    <div id="lista_archivos" class="space-y-2">
                        {{-- Mockup de archivos existentes --}}
                        @if($modo != 'create')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1">
                        ...
                        <p class="text-sm font-semibold p-2 rounded-lg 
                            @if($estado == 'Autorizado') bg-success-100 text-success-800 @endif
                            @if($estado == 'En Proceso') bg-warning-100 text-warning-800 @endif
                            @if($estado == 'Pagado') bg-blue-100 text-blue-800 @endif
                            @if($estado == 'Rechazado') bg-danger-100 text-danger-800 @endif
                        ">
                            {{ $estado ?? 'En Proceso' }}
                        </p>
                    </div>
                    {{-- Aquí irían más campos de auditoría si fuera necesario --}}
                </div>
            </div>
            @endif
            ...
            <div class="flex items-center justify-between pt-6 border-t border-secondary-200">
                <a href="{{ route('reintegros.index') }}" class="inline-flex items-center px-4 py-2 border border-secondary-300 rounded-lg text-sm font-medium text-secondary-700 bg-white hover:bg-secondary-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
