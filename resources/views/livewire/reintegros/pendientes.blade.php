<div class="mx-auto px-4" x-data="{ showDetail: @entangle('reintegroSeleccionado'), showInfoModal: @entangle('solicitandoInformacion'), showRechazoModal: @entangle('rechazandoReintegro') }">
    <!-- Header Section -->
            ...
            <table class="min-w-full divide-y divide-secondary-200">
                ...
                <tbody class="bg-white divide-y divide-secondary-200">
                    @forelse ($reintegros as $reintegro)
                        <tr class="hover:bg-secondary-50 transition-colors duration-150 @if(!$reintegro['es_nuevo']) bg-blue-50 @endif" wire:key="reintegro-{{ $reintegro['id'] }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-secondary-900 flex items-center">
                                    @if($reintegro['es_nuevo'])
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-secondary-900">{{ $reintegro['alumno'] }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    <button wire:click="verDetalle('{{ $reintegro['id'] }}')" class="p-2 text-secondary-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors duration-200" title="Ver detalles">
        ...
        @if ($reintegroSeleccionado)
            <div class="h-full flex flex-col">
                <div class="flex justify-between items-center px-6 py-4 bg-secondary-50 border-b border-secondary-200">
                    <h2 class="text-lg font-semibold text-secondary-900">Detalle del Reintegro: {{ $reintegroSeleccionado['id'] }}</h2>
                ...
                <div class="flex-grow p-6 overflow-y-auto">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div><span class="font-medium text-secondary-600">ID Accidente:</span> <span class="text-secondary-900">{{ $reintegroSeleccionado['accidente_id'] }}</span></div>
                            <div><span class="font-medium text-secondary-600">Alumno:</span> <span class="text-secondary-900">{{ $reintegroSeleccionado['alumno'] }}</span></div>
                            <div><span class="font-medium text-secondary-600">Escuela:</span> <span class="text-secondary-900">{{ $reintegroSeleccionado['escuela'] }}</span></div>
                            <div><span class="font-medium text-secondary-600">Fecha Solicitud:</span> <span class="text-secondary-900">{{ \Carbon\Carbon::parse($reintegroSeleccionado['fecha_solicitud'])->format('d/m/Y') }}</span></div>
                            <div class="md:col-span-2"><span class="font-medium text-secondary-600">Monto Solicitado:</span> <span class="text-secondary-900 font-bold text-lg">$ {{ number_format($reintegroSeleccionado['monto'], 2, ',', '.') }}</span></div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-3">
                            @forelse ($reintegroSeleccionado['documentos'] as $doc)
                                <li class="flex items-center justify-between p-3 bg-secondary-50 rounded-lg border border-secondary-200">
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-sm font-medium text-secondary-800">{{ $doc['nombre'] }}</span>
                                    </div>
                                    <a href="{{ $doc['url'] }}" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-white border border-secondary-300 rounded-md text-xs font-medium text-secondary-700 hover:bg-secondary-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                                        Ver
                                    </a>
                                </li>
                            @empty
    ...
    <div x-show="showInfoModal"
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 m-4" @click.away="showInfoModal = false">
            ...
            <div wire:key="solicitud-info-{{ $reintegroSeleccionado['id'] ?? 'new' }}">
    ...
    <div x-show="showRechazoModal"
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 m-4" @click.away="showRechazoModal = false">
            ...
            <div wire:key="rechazo-{{ $reintegroSeleccionado['id'] ?? 'new' }}">
