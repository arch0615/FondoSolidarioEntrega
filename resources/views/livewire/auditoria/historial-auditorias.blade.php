                ...
                <tbody class="bg-white divide-y divide-secondary-200">
                    @forelse ($reintegros as $reintegro)
                        <tr class="hover:bg-secondary-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-secondary-900">{{ $reintegro['escuela'] }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-secondary-500">
                                {{ $reintegro['codigo_accidente'] }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-secondary-900 font-medium">
                                ${{ number_format($reintegro['monto_solicitado'], 2) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-secondary-500">
                                {{ \Carbon\Carbon::parse($reintegro['fecha_solicitud'])->format('d/m/Y') }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-secondary-500">
                                @if ($reintegro['estado'] == 'Aprobado')
                                    <div><strong>Fecha Aprobación:</strong> {{ $reintegro['fecha_aprobacion'] }}</div>
                                @elseif ($reintegro['estado'] == 'Rechazado')
                                    <div><strong>Motivo:</strong> {{ $reintegro['motivo_rechazo'] }}</div>
                                    <div><strong>Fecha Rechazo:</strong> {{ $reintegro['fecha_rechazo'] }}</div>
                                @elseif ($reintegro['estado'] == 'Solicitud de Información')
                                    <div><strong>Solicitud:</strong> {{ $reintegro['solicitud_informacion'] }}</div>
                                    <div><strong>Fecha Solicitud:</strong> {{ $reintegro['fecha_solicitud'] }}</div>
                                    <div><strong>Días Transcurridos:</strong> {{ $reintegro['dias_transcurridos'] }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    <button wire:click="showReintegroModal({{ $reintegro['id'] }})" class="p-2 text-secondary-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors duration-200" title="Ver reintegro">
                ...
                <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div><strong class="text-secondary-600">Alumno:</strong> {{ $selectedReintegro['nombre_alumno'] }}</div>
                            <div><strong class="text-secondary-600">Escuela:</strong> {{ $selectedReintegro['escuela'] }}</div>
                            <div><strong class="text-secondary-600">Cód. Accidente:</strong> {{ $selectedReintegro['codigo_accidente'] }}</div>
                            <div><strong class="text-secondary-600">Monto Solicitado:</strong> ${{ number_format($selectedReintegro['monto_solicitado'], 2) }}</div>
                            <div><strong class="text-secondary-600">Estado:</strong> {{ $selectedReintegro['estado'] }}</div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm text-secondary-700 bg-secondary-50 p-4 rounded-lg">{{ $selectedReintegro['info_accidente'] }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-2">
                            @forelse($selectedReintegro['documentos'] as $doc)
                            <li class="flex items-center justify-between p-3 bg-secondary-50 rounded-lg border border-secondary-200">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-sm text-secondary-800">{{ $doc['nombre'] }}</span>
                                </div>
                                <a href="{{ $doc['url'] }}" target="_blank" class="text-sm text-primary-600 hover:text-primary-800 font-medium">Ver</a>
                            </li>
                            @empty
