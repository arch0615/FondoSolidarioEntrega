...
<div class="mx-auto px-4">
    ...
    <div class="mb-8">
        <nav class="flex items-center text-sm text-secondary-500 mb-4">
            <a href="{{ route('escuelas.index') }}" class="hover:text-secondary-700">Escuelas</a>
            <svg class="mx-2 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
            <span class="text-secondary-900">{{ $modo == 'create' ? 'Nueva Escuela' : ($modo == 'edit' ? 'Editar Escuela' : 'Detalles de Escuela') }}</span>
        </nav>
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-secondary-900">
                    {{ $modo == 'create' ? 'Nueva Escuela' : ($modo == 'edit' ? 'Editar Escuela' : 'Detalles de Escuela') }}
                </h1>
                <p class="mt-1 text-sm text-secondary-600">
                    {{ $modo == 'create' ? 'Complete los datos para registrar una nueva escuela' : ($modo == 'edit' ? 'Modifique los datos de la escuela' : 'Información detallada de la escuela') }}
                </p>
            </div>
            @if($modo == 'show')
            <div class="flex space-x-3">
                <a href="{{ route('escuelas.edit', $escuela_id ?? 1) }}" class="inline-flex items-center px-4 py-2 border border-secondary-300 rounded-lg text-sm font-medium text-secondary-700 bg-white hover:bg-secondary-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
    ...
    <div class="bg-white rounded-xl border border-secondary-200">
        <form action="{{ $modo == 'create' ? route('escuelas.index') : route('escuelas.index') }}" method="POST" class="space-y-6 p-6">
            @csrf
            @if($modo == 'edit')
                @method('PUT')
            @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1">
                        ...
                        <input
                            ...
                            value="{{ $escuela_id ?? '' == 1 ? 'ESC001' : ($escuela_id ?? '' == 2 ? 'ESC002' : '') }}"
                            {{ $modo == 'show' ? 'readonly' : '' }}
                            required
                        >
                    ...
                    <div class="space-y-1">
                        ...
                        <input
                            ...
                            value="{{ $escuela_id ?? '' == 1 ? 'Colegio San Martín' : ($escuela_id ?? '' == 2 ? 'Instituto Belgrano' : '') }}"
                            {{ $modo == 'show' ? 'readonly' : '' }}
                            required
                        >
                    </div>
                    ...
                    <div class="md:col-span-2 space-y-1">
                        ...
                        <input
                            ...
                            value="{{ $escuela_id ?? '' == 1 ? 'Av. Siempre Viva 742, Córdoba' : ($escuela_id ?? '' == 2 ? 'Calle Falsa 123, Córdoba' : '') }}"
                            {{ $modo == 'show' ? 'readonly' : '' }}
                            required
                        >
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1">
                        ...
                        <input
                            ...
                            value="{{ $escuela_id ?? '' == 1 ? '(351) 455-1234' : ($escuela_id ?? '' == 2 ? '(351) 477-5678' : '') }}"
                            {{ $modo == 'show' ? 'readonly' : '' }}
                            required
                        >
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <input
                            ...
                            value="{{ $escuela_id ?? '' == 1 ? 'info@sanmartin.edu.ar' : ($escuela_id ?? '' == 2 ? 'contacto@belgrano.edu.ar' : '') }}"
                            {{ $modo == 'show' ? 'readonly' : '' }}
                            required
                        >
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="relative">
                            ...
                            <input
                                ...
                                class="block w-full pl-8 pr-3 py-2 border border-secondary-300 rounded-lg text-sm placeholder-secondary-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 {{ $modo == 'show' ? 'bg-secondary-50' : 'bg-white' }}"
                                placeholder="150.50"
                                value="{{ $escuela_id ?? '' == 1 ? '250.75' : ($escuela_id ?? '' == 2 ? '180.00' : '') }}"
                                {{ $modo == 'show' ? 'readonly' : '' }}
                                required
                            >
                        </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <input
                            ...
                            value="{{ $escuela_id ?? '' == 1 ? '2023-01-15' : ($escuela_id ?? '' == 2 ? '2022-05-20' : '') }}"
                            readonly
                        >
                ...
                <div class="flex items-center">
                    <div class="flex items-center h-5">
                        <input
                            ...
                            {{ ($modo == 'create' || ($escuela_id ?? '' == 1 && $modo != 'create')) ? 'checked' : '' }}
                            {{ $modo == 'show' ? 'disabled' : '' }}
                        >
                    </div>
                    <div class="ml-3">
            ...
            <div class="flex items-center justify-between pt-6 border-t border-secondary-200">
                <a href="{{ route('escuelas.index') }}" class="inline-flex items-center px-4 py-2 border border-secondary-300 rounded-lg text-sm font-medium text-secondary-700 bg-white hover:bg-secondary-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button type="submit" class="inline-flex items-center px-6 py-2 bg-primary-600 border border-transparent rounded-lg font-medium text-sm text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
                    ...
                    {{ $modo == 'create' ? 'Crear Escuela' : 'Actualizar Escuela' }}
                </button>
                @endif
            </div>
        </form>
