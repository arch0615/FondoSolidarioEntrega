<div>
            ...
            <div class="bg-white rounded-xl border border-secondary-200 shadow-sm hover:shadow-lg transition-shadow duration-300 flex flex-col">
                <div class="p-6 flex-grow">
                    <div class="flex justify-between items-start mb-4">
                        <div class="flex-1 min-w-0 pr-3">
                            ...
                            <div class="mt-1">
                                ...
                                @elseif($accidente->alumnos->count() === 1)
                                    <p class="text-sm text-secondary-600 font-medium">{{ $accidente->alumnos->first()->alumno->nombre_completo ?? 'Alumno no encontrado' }}</p>
                                @else
                                    <p wire:click="getAlumnosDelAccidente({{ $accidente->id_accidente }})" class="text-sm text-secondary-600 font-medium whitespace-nowrap cursor-pointer hover:text-primary-600 transition-colors duration-200">
                                        {{ $accidente->alumnos->first()->alumno->nombre_completo ?? 'Alumno no encontrado' }}
                                        <span class="text-xs text-secondary-500 underline">+{{ $accidente->alumnos->count() - 1 }} más</span>
                                    </p>
                                    <p class="text-xs text-secondary-500 mt-1 whitespace-nowrap">
                                        <strong>Total:</strong> {{ $accidente->alumnos->count() }} alumnos involucrados
                                    </p>
                                @endif
                            </div>
                            <p class="text-xs text-secondary-500 mt-1">{{ $accidente->escuela->nombre }}</p>
                        </div>
                        <div class="flex items-center space-x-2">
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-success-100 text-success-800">
                                ...
                                {{ $accidente->estado->nombre_estado }}
                            </span>
                            @endif
                            ...
                            <div x-data="{ open: false }" @click.away="open = false" class="relative">
                                ...
                                <div x-show="open"
                                     ...
                                     style="display: none;">
                                    <a href="{{ route('accidentes.show', $accidente->id_accidente) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-secondary-700 hover:bg-secondary-100 hover:text-secondary-900 w-full text-left">
                    ...
                    <div class="mb-4 text-sm text-secondary-700 space-y-1">
                        <p><i class="fas fa-calendar-alt text-secondary-400 mr-2 fa-fw"></i>{{ $accidente->fecha_accidente->format('d/m/Y') }} - {{ $accidente->hora_accidente ? $accidente->hora_accidente->format('H:i') : 'N/A' }}</p>
                        <p><i class="fas fa-map-marker-alt text-secondary-400 mr-2 fa-fw"></i>{{ $accidente->lugar_accidente }}</p>
                    ...
                    <div class="border-t border-secondary-200 pt-4 mb-4">
                        ...
                        <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                            <p class="text-secondary-700">Derivaciones: <span class="font-semibold text-blue-600">{{ $accidente->derivaciones->count() }}</span></p>
                            <p class="text-secondary-700">Reintegros: <span class="font-semibold text-secondary-900">{{ $accidente->reintegros->count() }}</span></p>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-2 mt-4">
                        ...
                        <a href="#" class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-green-600 border border-transparent rounded-lg font-medium text-xs text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200">
        ...
        <div class="mt-8 px-6 py-4 bg-white border-t border-secondary-200 flex flex-col sm:flex-row items-center justify-between rounded-b-xl">
            @if($accidentes->hasPages())
                <div class="text-sm text-secondary-700">
                    Mostrando {{ $accidentes->firstItem() }} a {{ $accidentes->lastItem() }} de {{ $accidentes->total() }} resultados
                </div>
                {{ $accidentes->links('pagination.custom-tailwind') }}
            @else
                <div class="text-sm text-secondary-700">
    ...
    function exportarAccidentes(formato) {
        ...
        switch(formato) {
            case 'csv':
                url = '{{ route("accidentes.export.csv") }}';
                break;
            case 'excel':
                url = '{{ route("accidentes.export.excel") }}';
                break;
            case 'pdf':
                url = '{{ route("accidentes.export.pdf") }}';
                break;
        }
