...
<div class="mx-auto px-4">
    ...
    <div class="mb-8">
        <nav class="flex items-center text-sm text-secondary-500 mb-4">
            {{-- TODO: Definir la ruta correcta para el listado de derivaciones --}}
            <a href="{{ route('derivaciones.index') }}" class="hover:text-secondary-700">Derivaciones</a>
            <svg class="mx-2 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
            <span class="text-secondary-900">{{ $modo == 'create' ? 'Nueva Derivación' : ($modo == 'edit' ? 'Editar Derivación' : 'Detalles de Derivación') }}</span>
        </nav>
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-secondary-900">
                    {{ $modo == 'create' ? 'Nueva Derivación' : ($modo == 'edit' ? 'Editar Derivación' : 'Detalles de Derivación') }}
                </h1>
                <p class="mt-1 text-sm text-secondary-600">
                    {{ $modo == 'create' ? 'Complete los datos para registrar una nueva derivación médica.' : ($modo == 'edit' ? 'Modifique los datos de la derivación médica.' : 'Información detallada de la derivación médica.') }}
                </p>
            </div>
            @if($modo == 'show')
            <div class="flex space-x-3">
                {{-- TODO: Definir la ruta correcta para editar derivación --}}
                <a href="{{ route('derivaciones.edit', $derivacion_id ?? 1) }}" class="inline-flex items-center px-4 py-2 border border-secondary-300 rounded-lg text-sm font-medium text-secondary-700 bg-white hover:bg-secondary-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
    ...
    <div class="bg-white rounded-xl border border-secondary-200">
        ...
        <form action="{{ $modo == 'create' ? route('derivaciones.index') : route('derivaciones.index') }}" method="POST" class="space-y-6 p-6" enctype="multipart/form-data">
            @csrf
            @if($modo == 'edit')
                @method('PUT')
            @endif
            ...
            <div class="border-b border-secondary-200 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1">
                        ...
                        {{-- TODO: Implementar selección de accidente dinámico --}}
                        <select
                            name="id_accidente"
                            id="id_accidente"
                        ...
                        >
                            ...
                            <option value="1" {{ ($derivacion_id ?? '' == 1) ? 'selected' : '' }}>ACC-001 (Juan Pérez - Colegio San Martín)</option>
                            <option value="2" {{ ($derivacion_id ?? '' == 2) ? 'selected' : '' }}>ACC-002 (Ana López - Instituto Belgrano)</option>
                        </select>
                        @if($modo == 'create')
                        <p class="text-xs text-secondary-500">Si el accidente no está listado, primero debe <a href="{{ route('accidentes.create') }}" class="text-primary-600 hover:underline">registrar el accidente</a>.</p>
                        @endif
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        {{-- TODO: Implementar selección de prestador dinámico --}}
                        <select
                            name="id_prestador"
                            id="id_prestador"
                            class="block w-full px-3 py-2 border border-secondary-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 {{ $modo == 'show' ? 'bg-secondary-50' : 'bg-white' }}"
                        ...
                        >
                            ...
                            <option value="1" {{ ($derivacion_id ?? '' == 1) ? 'selected' : '' }}>Clínica del Sol</option>
                            <option value="2" {{ ($derivacion_id ?? '' == 2) ? 'selected' : '' }}>Hospital Privado</option>
                    ...
                    <div class="space-y-1">
                        ...
                        <input
                            ...
                            value="{{ $derivacion_id ?? '' == 1 ? '2024-05-15' : ($derivacion_id ?? '' == 2 ? '2024-05-18' : date('Y-m-d')) }}"
                            {{ $modo == 'show' ? 'readonly' : '' }}
                            required
                        >
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <input
                            ...
                            id="hora_derivacion"
                            class="block w-full px-3 py-2 border border-secondary-300 rounded-lg text-sm {{ $modo == 'show' ? 'bg-secondary-50' : 'bg-white' }}"
                            value="{{ $derivacion_id ?? '' == 1 ? '10:30' : ($derivacion_id ?? '' == 2 ? '15:15' : date('H:i')) }}"
                            {{ $modo == 'show' ? 'readonly' : '' }}
                            required
                        >
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <input
                            ...
                            value="{{ $derivacion_id ?? '' == 1 ? 'Dr. Lucas Gonzalez (SEM)' : ($derivacion_id ?? '' == 2 ? 'Dra. Valeria Ochoa (SEM)' : '') }}"
                            {{ $modo == 'show' ? 'readonly' : '' }}
                        >
                    ...
                    <div class="space-y-1">
                        ...
                        <input
                            ...
                            value="{{ $derivacion_id ?? '' == 1 ? 'Mariana Torres (Preceptora)' : ($derivacion_id ?? '' == 2 ? 'Roberto Sánchez (Padre)' : '') }}"
                            {{ $modo == 'show' ? 'readonly' : '' }}
                            required
                        >
                    ...
                    <div class="md:col-span-2 space-y-1">
                        ...
                        <textarea
                            ...
                            {{ $modo == 'show' ? 'readonly' : '' }}
                            required
                        >{{ $derivacion_id ?? '' == 1 ? 'Golpe en la cabeza, posible contusión leve.' : ($derivacion_id ?? '' == 2 ? 'Dolor abdominal agudo, se sugiere observación.' : '') }}</textarea>
                    ...
                    <div class="md:col-span-2 space-y-1">
                        ...
                        <textarea
                            ...
                            {{ $modo == 'show' ? 'readonly' : '' }}
                        >{{ $derivacion_id ?? '' == 1 ? 'Se administró paño frío. Familia notificada.' : ($derivacion_id ?? '' == 2 ? 'Presentó fiebre leve (37.8°C) antes de la derivación.' : '') }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    <div class="flex items-center h-5">
                        <input
                            ...
                            {{ ($derivacion_id ?? '' == 1 && $modo != 'create') ? 'checked' : '' }}
                            disabled {{-- Este campo es solo informativo aquí --}}
                        >
                    </div>
                    <div class="ml-3">
                        ...
                        <p class="text-xs text-secondary-500">Indica si el documento de derivación ya ha sido impreso. (Fecha de impresión: {{ $derivacion_id ?? '' == 1 ? '15/05/2024 10:35 AM' : 'No impresa' }})</p>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="flex items-center justify-between pt-6 border-t border-secondary-200">
                {{-- TODO: Definir la ruta correcta para el listado de derivaciones --}}
                <a href="{{ route('derivaciones.index') }}" class="inline-flex items-center px-4 py-2 border border-secondary-300 rounded-lg text-sm font-medium text-secondary-700 bg-white hover:bg-secondary-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button type="submit" class="inline-flex items-center px-6 py-2 bg-primary-600 border border-transparent rounded-lg font-medium text-sm text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
                    ...
                    {{ $modo == 'create' ? 'Crear Derivación' : 'Actualizar Derivación' }}
                </button>
                @endif
            </div>
        </form>
