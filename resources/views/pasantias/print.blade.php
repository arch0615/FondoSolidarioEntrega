...
<head>
    ...
    <title>Pasantía - {{ $pasantia->alumno->apellido ?? '' }} {{ $pasantia->alumno->nombre ?? '' }}</title>
...
<body>
    <div class="no-print my-4 text-center">
        <a href="{{ route('pasantias.pdf', $pasantia->id_pasantia) }}" target="_blank" class="inline-block bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg shadow-md">
            Descargar PDF
        </a>
    ...
    <div class="page">
        ...
        <main class="text-sm leading-normal">
            <p class="mb-4">Por medio de la presente, se deja constancia de la siguiente pasantía registrada por la <strong>{{ $pasantia->escuela->nombre ?? 'N/A' }}</strong>:</p>
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-md p-4 mb-4">
                ...
                <table class="w-full text-sm">
                    <tr>
                        ...
                        <td class="py-1">{{ $pasantia->alumno->apellido ?? '' }} {{ $pasantia->alumno->nombre ?? '' }}</td>
                    </tr>
                    <tr>
                        <td class="font-semibold py-1 pr-4">DNI:</td>
                        <td class="py-1">{{ $pasantia->alumno->dni ?? 'N/A' }}</td>
                    </tr>
                </table>
            </div>
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-md p-4 mb-4">
                ...
                <table class="w-full text-sm">
                    <tr>
                        ...
                        <td class="py-1">{{ $pasantia->empresa }}</td>
                    </tr>
                    <tr>
                        <td class="font-semibold py-1 pr-4">Dirección:</td>
                        <td class="py-1">{{ $pasantia->direccion_empresa }}</td>
                    </tr>
                    <tr>
                        <td class="font-semibold py-1 pr-4">Tutor de Empresa:</td>
                        <td class="py-1">{{ $pasantia->tutor_empresa }}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td class="py-1">{{ $pasantia->horario }}</td>
                    </tr>
                    @endif
                </table>
            </div>
