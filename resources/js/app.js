    ...
    utils: {
        ...
        showNotification: (message, type = 'info', duration = 5000) => {
            ...
            if (!container) {
                ...
                container.className = 'fixed top-4 right-4 z-[9999] flex flex-col gap-2 w-80 pointer-events-none';
            ...
            toast.innerHTML = `
                <svg class="h-5 w-5 flex-shrink-0 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
                    ${svgPaths[type] || svgPaths.info}
                </svg>
                <p class="text-sm font-medium flex-1">${message}</p>
                <button class="flex-shrink-0 opacity-75 hover:opacity-100 transition-opacity ml-1">
